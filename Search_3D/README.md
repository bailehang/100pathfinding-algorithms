...
| B01 | Minimum Snap | 轨迹优化 | 近似实现 | 离散四阶差分 (snap) 梯度优化 + ESDF 推离，非多项式 QP |
| B02 | Minimum Jerk | 轨迹优化 | 近似实现 | 离散三阶差分 (jerk) 梯度优化 + ESDF 推离 |
| B03 | SFC + Convex QP | 轨迹优化 | 近似实现 | 管状走廊约束内的平滑优化（越界投影回走廊）+ 走廊可视化 |
...
| C02 | RVO 3D | 分布式避让 | 近似实现 | 互惠速度障碍：apex=(vA+vB)/2 的速度障碍投影 |
| C03 | Buffered Voronoi Cells | 分布式避让 | 近似实现 | 缓冲 Voronoi 胞：逐邻居半平面速度约束顺序投影 |
| C04 | DMPC | 分布式避让 | 近似实现 | 采样式滚动优化：候选速度 × 常速预测 rollout 代价择优 |
| C05 | MADER / RMADER | 分布式避让 | 近似实现 | 异步 commit + check/recheck 协议，含近距硬分离保护 |
| C06 | EGO-Swarm | 分布式避让 | 近似实现 | 广播轨迹冲突预测 + 侧向绕行 + ESDF 梯度避障 |
| C07 | DCP / 错峰通过 | 分布式避让 | 近似实现 | 已接入时空保留表调度，按体素-时间冲突自动追加延迟错峰通过 |
...
| D03 | Olfati-Saber Flocking | 反应式 / 场方法 | 近似实现 | σ-norm α-agent 梯度项 + 速度一致项（邻居数归一化 + 近距硬壳增强） |
| D04 | Vasarhelyi Flocking | 反应式 / 场方法 | 近似实现 | 线性短程排斥 + 制动曲线速度对齐（论文核心项） |
...
| F01 | MAPF 3D | 集中式协调 | 近似实现 | 时空保留表调度：逐机延迟起飞至无体素-时隙冲突 |
| F02 | CBS / ECBS | 集中式协调 | 近似实现 | CBS-lite：迭代定位最早冲突并贪心延迟代价较小一方 |
| F03 | PBS | 集中式协调 | 近似实现 | 优先级规划：按路径长度排序 + 带缓冲的顺序保留表 |
| F04 | SCP 集中式轨迹 | 集中式协调 | 近似实现 | 时间采样轨迹的集中式迭代解冲突 + 平滑 + 障碍投影 |
| F05 | 匈牙利 / 拍卖分配 | 集中式协调 | 近似实现 | 拍卖算法目标分配 + 单目标 A*（>300 架时简化直线路由） |
