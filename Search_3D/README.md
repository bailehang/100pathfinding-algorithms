...
| A02 | Hybrid A* | 搜索 / 采样 | 已实现 | 16 航向 Hybrid A*：连续弧段运动基元 + 爬升动作 + 曲率/转向代价 + 弧段碰撞检查 |
| A03 | JPS 3D | 搜索 / 采样 | 已实现 | 3D JPS：方向剪枝 + 自然邻居 + forced-neighbor + 递归投影跳跃 + no-corner-cut |
| A04 | RRT* | 搜索 / 采样 | 已实现 | RRT*：动态近邻半径 + 最优父节点 + rewire 代价传播 + 分支剪枝 |
| A05 | Informed RRT* | 搜索 / 采样 | 已实现 | RRT* 首解后 prolate hyperspheroid informed 采样 + 当前最优代价剪枝 |
| A06 | BIT* | 搜索 / 采样 | 已实现 | 批量 informed 采样 + 顶点/边队列 + 惰性碰撞检测 + 批次剪枝 |
| A07 | Kinodynamic A* | 搜索 / 采样 | 已实现 | (体素 × 速度) 状态搜索：27 加速度动作 + 加速度/jerk 代价 + 动力学可达扩展 |
| A08 | Motion Primitives | 搜索 / 采样 | 已实现 | 运动基元库 A*：二阶速度拼接弧段 + 连续曲线碰撞检查 + 曲率代价 |
...
| B04 | B-Spline + ESDF | 轨迹优化 | 已实现 | 三次均匀 B 样条控制点优化：平滑/导向/ESDF 障碍/控制点间距约束 + 净空投影 |
| B05 | Fast-Planner | 轨迹优化 | 已实现 | Kinodynamic A* 前端 + B 样条 ESDF 后端 + 曲率/净空感知时间重分配 |
| B06 | GCOPTER / MINCO | 轨迹优化 | 已实现 | MINCO 风格稀疏路标与段时长联合优化 + 闭式 minimum-snap 多项式重建 |
...
| E01 | GLAS | 学习类 | 已实现 | 固定权重 GLAS 风格局部观测网络：k 近邻注意力聚合 + 危险度安全混合 |
| E02 | PRIMAL / PRIMAL2 | 学习类 | 已实现 | 局部观测编码 + 固定权重动作评分网络，在 26 个移动动作 + 等待动作中选择 |
| E03 | Neural CBF | 学习类 | 已实现 | 神经屏障裕度/α 估计 + 邻机/障碍 CBF 约束的 QP 顺序投影滤波 |
| E04 | RL + Safety Layer | 学习类 | 已实现 | 固定权重 RL 策略网络输出动作，安全层通过短时 rollout 与价值评估选择安全动作 |
| E05 | End-to-End Swarm RL | 学习类 | 已实现 | 端到端 CTDE MLP：k 近邻局部观测直接输出油门与三轴速度修正，并叠加分离保护 |
