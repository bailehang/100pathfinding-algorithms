...
class MultiAgentGCS:
    ...
    def save_gif(self):
        for agent_id, region_paths, routes in self.snapshots:
            ...
            self.world.draw(ax, f"098 MGCS - sequential convex-set coupling agent {agent_id}")
        ...
        for alpha in [0.25, 0.45, 0.65, 0.85, 1.0]:
            fig, ax = plt.subplots(figsize=self.recorder.figsize, dpi=self.recorder.dpi)
            self.world.draw(ax, "098 MGCS - optimized multi-agent convex corridors")
            draw_agent_routes(ax, self.starts, self.goals, self.routes, upto=alpha)
            self.recorder.capture()
            plt.close(fig)
        self.recorder.save("098_MGCS", fps=3, hold=5)
