"""
SPFA 2D pathfinding demo.

SPFA, the Shortest Path Faster Algorithm, is a queue-based optimization of
Bellman-Ford. It only propagates from nodes whose distance improved.
"""
...
class SPFA:
    """Queue-based shortest path search on an 8-connected grid."""
    ...
    def searching(self):
        ...
        self.relax_count = {node: 0 for node in self.nodes}
        self.dist[self.s_start] = 0.0

        queue = deque([self.s_start])
        in_queue = {self.s_start}
        ...
        while queue:
            ...
            for neighbor in self.get_neighbors(current):
                new_cost = self.dist[current] + self.motion_cost(current, neighbor)
                if new_cost >= self.dist[neighbor]:
                    continue

                self.dist[neighbor] = new_cost
                self.parent[neighbor] = current
                self.relax_count[neighbor] += 1

                if self.relax_count[neighbor] >= len(self.nodes):
...
class PlottingSPFA:
    ...
    def draw_visited(self, draw, visited):
        for node in visited:
            self.draw_cell(draw, node, (150, 220, 185, 150))
...
def main():
    print("SPFA Pathfinding Demo")
    print("=" * 40)

    s_start = (5, 5)
    s_goal = (45, 25)

    planner = SPFA(s_start, s_goal)
    path, visited = planner.searching()
    print_latest_metrics()

    print(f"Start: {s_start}")
    print(f"Goal: {s_goal}")
    print(f"Dequeued nodes: {len(visited)}")
    ...
    plot = PlottingSPFA(planner)
    plot.animation(path, visited, "006_SPFA", save_gif=True)
