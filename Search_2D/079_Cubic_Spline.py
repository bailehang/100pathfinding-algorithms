...
class CubicSplinePlanner(GridCurveDemo):
    ...
    def run_demonstration(self):
        ...
        self.draw_search_frame("074 Cubic Spline - Grid Search", self.visited, self.raw_path)
        self.draw_search_frame("074 Cubic Spline - Spline Anchors", [], self.raw_path, self.spline_anchors)

        for i in range(1, 13):
            upto = max(2, int(len(self.spline_path) * i / 12))
            visible = self.spline_path[:upto]
            self.draw_base("074 Cubic Spline - Natural Cubic Trajectory")
            plt.plot(
        ...
        self.save_gif("079_Cubic_Spline", fps=4)
        plt.close("all")
