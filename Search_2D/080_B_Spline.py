...
class BSplinePlanner(GridCurveDemo):
    ...
    def run_demonstration(self):
        ...
        self.draw_search_frame("075 B-Spline - Grid Search", self.visited, self.raw_path)
        self.draw_search_frame("075 B-Spline - Control Polygon", [], self.raw_path, self.control_points)

        for i in range(1, 13):
            upto = max(2, int(len(self.bspline_path) * i / 12))
            visible = self.bspline_path[:upto]
            self.draw_base("075 B-Spline - Smooth Basis Curve")
            plt.plot(
        ...
        self.save_gif("080_B_Spline", fps=4)
        plt.close("all")
