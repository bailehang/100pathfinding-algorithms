...
class ReedsSheppCurves(GridCurveDemo):
    ...
    def run_demonstration(self):
        ...
        self.draw_search_frame("078 Reeds-Shepp - Grid Search", self.visited, self.raw_path)
        self.draw_search_frame("078 Reeds-Shepp - Forward/Reverse Anchors", [], self.raw_path, self.waypoints)

        for i in range(1, 13):
            upto = max(2, int(len(self.rs_path) * i / 12))
            self.draw_base("078 Reeds-Shepp Curves - Bidirectional Path")
            plt.plot(
        ...
        self.save_gif("083_Reeds_Shepp_Curves", fps=4)
        plt.close("all")
