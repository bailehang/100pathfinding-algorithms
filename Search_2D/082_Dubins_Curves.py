...
class DubinsCurves(GridCurveDemo):
    ...
    def run_demonstration(self):
        ...
        self.draw_search_frame("077 Dubins Curves - Grid Search", self.visited, self.raw_path)
        self.draw_search_frame("077 Dubins Curves - Waypoint Skeleton", [], self.raw_path, self.waypoints)

        for i in range(1, 13):
            upto = max(2, int(len(self.dubins_path) * i / 12))
            visible = self.dubins_path[:upto]
            self.draw_base("077 Dubins Curves - Forward Line-Arc-Line Path")
            plt.plot(
        ...
        self.save_gif("082_Dubins_Curves", fps=4)
        plt.close("all")
